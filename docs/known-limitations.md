...
| PDF sources             | Authenticated, blob-backed, browser-viewer, or policy-blocked PDF sources may not allow original-byte passthrough.                  | Use the dedicated PDF viewer capture when available, download the PDF manually, or grant the exact optional origin/file permission. |
