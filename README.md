...
**S19 — diagnostics, localization, privacy, permissions, and accessibility is implemented.** WebCap now provides persisted Vietnamese and English copy across the popup, PDF editor, region selector, element selector, restricted-page guidance, permission rationale, and every user-facing error code. Copyable diagnostics use an explicit technical allowlist and exclude URLs, page text, selectors, credentials, tokens, cookies, and binary content; production logging defaults to `warn` with the same safe-context boundary. Privacy and dependency audits now run permanently in read-only CI and verify no remote executable code, analytics SDK, unsafe diagnostics fields, default host permission, or incompatible direct dependency license. Keyboard, live-region, semantic-label, reduced-motion, locale-persistence, diagnostics-copy, and restricted-page browser flows are covered without adding telemetry, accounts, cloud sync, a backend, or new required permissions. The clean S19 gate passes 276 unit tests across 78 files, four PDF benchmarks, the verified Manifest V3 production build, and 35 Playwright E2E cases. S20 is next: release-candidate packaging, clean-profile install/update/uninstall checks, release documentation, and Chrome Web Store readiness.
