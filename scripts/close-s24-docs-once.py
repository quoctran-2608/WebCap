...
replacements = {
    "date: 2026-08-04\n": "date: 2026-08-05\n",
    "current_session: S24\n": "current_session: S25\n",
    "S21–S23 đã hoàn tất reset lifecycle, region selector đáng tin cậy và adaptive auto-scroll có resumable frontier mà không thay manifest, package version hoặc artifact 0.1.0. S24 là session active tiếp theo.": "S21–S24 đã hoàn tất reset lifecycle, region selector đáng tin cậy, adaptive auto-scroll có resumable frontier và mode-aware output mà không thay manifest, package version hoặc artifact 0.1.0. S25 là session active tiếp theo.",
    "| S24 | Auto-PDF và mode-aware image/PDF output | PLANNED | S23 |": "| S24 | Auto-PDF và mode-aware image/PDF output | DONE | S23 |",
    "| S25 | Stored settings, event-driven progress và simplified popup | BLOCKED | S21–S24 stable contracts |": "| S25 | Stored settings, event-driven progress và simplified popup | PLANNED | S21–S24 stable contracts |",
}
for old, new in replacements.items():
    if old not in plan:
        raise SystemExit(f"Missing PLAN marker: {old}")
...
if "## S24 implementation evidence" in plan:
    raise SystemExit("S24 evidence already exists")
if marker not in plan:
    raise SystemExit("Missing S25 PLAN marker")
plan = plan.replace(marker, evidence + marker, 1)
plan_path.write_text(plan)

spec_path = Path("docs/spec-0.2.0.md")
spec = spec_path.read_text()
if "date: 2026-08-04\n" not in spec:
    raise SystemExit("Missing spec date marker")
spec = spec.replace("date: 2026-08-04\n", "date: 2026-08-05\n", 1)
spec_section = """
...
if "## 5.4 Locked S24 implementation semantics" in spec:
    raise SystemExit("S24 spec semantics already exist")
if spec_marker not in spec:
    raise SystemExit("Missing spec reset marker")
spec = spec.replace(spec_marker, spec_section + spec_marker, 1)
...
additions = """- Mode-aware completion routing that automatically creates PDF for full-page and scroll-area captures, returns guarded PNG/JPEG/WebP for region and element captures, and preserves the existing visible-image flow.
...
"""
if "Mode-aware completion routing that automatically creates PDF" in changelog:
    raise SystemExit("S24 changelog entries already exist")
changelog = changelog.replace(changelog_marker, changelog_marker + additions, 1)
...
new_status = """**0.2.0 implementation is active. S21 capture reset, S22 region-selector reliability, S23 adaptive auto-scroll and S24 automatic mode-aware output are complete; S25 settings/events/simplified popup is next.** Full-page and scroll-area captures now create PDF automatically, while region and element captures create guarded PNG/JPEG/WebP output. Durable result metadata survives popup reopen and service-worker restart, auto-generated PDFs remain editable without recapture, and oversized image output offers an explicit PDF fallback that reuses the same stored tiles. S25–S26 remain planned for stored settings, event-driven progress, popup simplification and release hardening."""
if old_status not in readme:
    raise SystemExit("Missing README current-status marker")
readme = readme.replace(old_status, new_status, 1)
readme = readme.replace(
    "## Planned 0.2.0 outcomes", "## 0.2.0 outcomes and remaining work", 1
)
readme_replacements = {
    "- One-click **Full page → PDF**: capture from document start, continue through finite lazy growth, restore the page and automatically create a PDF.": "- Delivered in S23–S24: one-click **Full page → PDF** captures from document start, follows finite lazy growth, restores the page and automatically creates a PDF.",
    "- A reliable **Draw region** flow: the popup closes only after the selector is ready, the overlay appears immediately, pointer and keyboard can create/adjust the rectangle, and selector UI is excluded from output.": "- Delivered in S22–S24: **Draw region** closes the popup only after selector readiness, supports pointer and keyboard editing, excludes selector UI and returns a guarded image result.",
    "- A clear **New capture** action that safely cancels or discards the current local job and allows another capture on the same tab.": "- Delivered in S21: **New capture** safely cancels or discards the current local job and allows another capture on the same tab.",
    "- Mode-aware output: region/element default to guarded image output; full page defaults to PDF; oversized images receive an explicit PDF fallback.": "- Delivered in S24: region/element default to guarded image output, full-page/scroll-area default to PDF, and oversized images receive an explicit no-recapture PDF fallback.",
}
for old, new in readme_replacements.items():
    if old not in readme:
        raise SystemExit(f"Missing README outcome marker: {old}")
