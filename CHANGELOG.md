...
- S30 Viewer Intelligence V2: evidence-driven pre-capture PDF viewer discovery for PDF.js-style, generic semantic, open-shadow-root, virtualized/recycled-DOM and guarded canvas-only viewers; logical pages are accumulated across settled scroll positions, stable declared page indices survive DOM recycling, mixed page sizes/orientations retain their observed geometry, and the selected container scroll position is restored after discovery.
- S30 completion hardening: a legacy uniform-rhythm `projected` page map is no longer sufficient viewer-completion evidence when incremental discovery runs; declared-count completion requires every logical page index plus stable start/end proof, geometry/visual fallback requires continuity/confidence evidence, ordinary scroll containers skip the expensive PDF discovery pass, and lazy viewer growth can expand capture planning from the verified page-map extent without weakening width-drift guards.
- S30 browser regression for a 500-page virtualized PDF fixture that keeps only a small moving page window in the DOM while discovery must still identify all 500 logical pages before capture.
- S29 PDF Source Acquisition V2: original PDF sources stream through an OPFS temporary spool with incremental `%PDF-` signature and SHA-256 verification instead of requiring a whole-document `Uint8Array`; embedded `embed`/`object`/`iframe`/`source`/link/blob candidates, credential-aware fetch, permission-checked redirects and optional active-tab CDP recovery preserve byte-identical originals when safely available and fail closed to viewer capture on storage/source failure.
- S28 dedicated PDF orchestration and contracts: versioned `PdfDocumentManifest`, page-first progress/state, strategy negotiation, dedicated PDF storage and verified completion evidence separate PDF completion semantics from generic tile completion.
- S27 dedicated page-aware PDF capture: detected logical source pages map one-to-one to output pages, normal tile limits become capture batching rather than document-end evidence, page-local export excludes viewer gutters/gaps, and source page size/orientation is preserved.
...
- Packaged clean-profile lifecycle validation for install, optional-host-permission absence, simulated update from 0.0.9 to 0.1.0 with stable extension ID and preserved `chrome.storage.local`, self-uninstall, and same-profile absence verification.
...
- Chrome Web Store handoff documents, bilingual listing copy, release checklist, known-limitations workarounds, release notes, changelog, and MUST acceptance-criteria traceability without creating a tag, GitHub Release, or store submission.
